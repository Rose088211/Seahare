<div align="center">
  ...
  <img src="screenshots/main.png" alt="Seahare 淡色玻璃主题主界面" width="800" />
...
<p align="center">
  ...
  <b>一款前后端分离的 Windows Web 目录与接口扫描工作台</b><br />
...
| **Web / API / H5** | 支持 Web 目录、JSON API 和 H5 路由回退识别，并共享请求配置           |
| **任务全生命周期** | 自定义名称、启动、暂停、恢复、取消、删除、中断恢复、一键复制重试       |
| **Projects 历史整理** | 全部任务、手动文件夹、目标主机、目标路径和扫描任务；支持重命名与拖放归档 |
...
| **四种界面主题** | 深色、浅色、深色玻璃、淡色玻璃；玻璃主题提供动态光场和背景模糊       |
...
<div align="center">
  ...
  <img src="screenshots/create-scan.png" alt="Seahare 淡色玻璃主题创建扫描任务" width="700" />
...
2. 输入任务名称（可选）和目标 URL
3. 选择扫描策略（快速 / 均衡 / 谨慎）
4. 选择目标类型、字典文件或切换为自定义枚举模式
5. 配置请求方法、请求头、请求体、线程数和超时
6. 点击 **启动扫描**
...
- 按响应类型筛选（全部 / 有效响应 / 重定向）
- 搜索路径、状态码或内容类型
- 展开结果证据，查看响应摘要、业务字段、哈希和 SPA 回退判断
- 导出结果为 CSV 文件

左侧历史栏采用 Projects 风格树状结构。扫描任务会按目标主机和目标路径自动分组；用户可以创建嵌套文件夹，拖动任务、分组或整个文件夹进行归档，也可以将内容拖到“全部任务”或空白区域移出文件夹。文件夹、任务名称、主机显示名称和主题选择保存在当前浏览器本地。

### 浮动工作区

<div align="center">

  <img src="screenshots/workspace.png" alt="Seahare 淡色玻璃主题浮动工作区" width="700" />
...
```text
seahare-theme                  当前界面主题
seahare-history-folders        文件夹层级与扫描任务归属
seahare-history-scan-names     扫描任务自定义名称
seahare-history-host-names     自动主机分组显示名称
```
